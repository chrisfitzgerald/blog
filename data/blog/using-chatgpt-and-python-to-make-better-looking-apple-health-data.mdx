...
Apple Health is a great app that allows users to track their physical activity, heart rate, and other health-related data. However, the app has limited visualization capabilities. For instance, the Heart Rate data mainly shows your heart rate on a single range bar. I am no data scientist, but I think the visualization capabilities for the heart rate data left something to be desired. Fortunately, Apple is kind enough to share this data, allowing us to get creative. In this post, I will share my experience using Python and OpenAI’s [ChatGPT](https://openai.com/) to create fast code that extracts my Heart Rate data and plots it using the Python libraries [Pandas](https://pandas.pydata.org/) and [matplotlib](https://matplotlib.org/).
...
I am learning to code and I like messing around with stuff that is already built (boilerplate, cloning repos, etc..). So using a AI chatbot to jumpstart an idea I have off the ground is thrilling for me. It not only fills out the blank page, but it exposes me to new libraries that I may not of otherwise been exposed to. Here is what ChatGPT told me to do when I prompted it: We import the xml.etree.ElementTree module is a simple and powerful tool for parsing XML data.  ChatGPT decides to import pandas to create a DataFrame and matplotlib to create our charts from this data:
```python
...
When ran, we now have an easier to look at chart with not only a line graph that plots my data, but also the ability to switch to scatter plots which I believe works well to express what this data represents. I have also dug into matplotlib’s documentation and found a built in gradient called ‘cool’ which we can define in the line `cmap = plt.get_cmap('cool')` to show the Y-axis with heightening intensity as my heart rate increases during exercise. 
